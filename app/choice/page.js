...
export default function SvgMatrixPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black relative overflow-hidden">
      ...
      {/* Tooltip Box */}
      {hovered === "files" && (
        <div className="absolute left-[20%] top-[35%] bg-cyan-800 text-white p-3 rounded shadow-md z-10 w-52 text-center">
          <div className="text-sm font-bold">FiloDrop</div>
          <div className="text-xs">Upload, protect, and share files with passwords and short URLs.</div>
        </div>
      )}
      {hovered === "filopad" && (
        <div className="absolute right-[20%] top-[35%] bg-red-800 text-white p-3 rounded shadow-md z-10 w-52 text-center">
          <div className="text-sm font-bold">FiloPad</div>
          <div className="text-xs">Paste code or text anonymously and share short URLs easily.</div>
        </div>
      )}
